...
// Write data to the host, no header is sent, data must be 64 bit aligned
int usbWriteRawBulkData(const void *data, int len);

// Read data from the host, no header, arbitrary alignment
int usbReadRawBulkData(void *data, int len);
